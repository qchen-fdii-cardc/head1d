...
def create_animation(result, output_file=None, interval=50, ymin=None, ymax=None, L=1.0):
    """Create an animation from simulation data with both line plot and heatmap."""
    if not result:
    ...
    if output_file is None:
        ...
        output_file = imgs_dir / f"heat_conduction_2d_{timestamp}.gif"
    else:
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8), height_ratios=[1, 0.5])
    line, = ax1.plot([], [], lw=2)
    ...
    ax1.text(0.98, 0.95, param_text, transform=ax1.transAxes, 
    ...
    ax1.set_title("Heat Conduction Simulation")
    ax1.grid(True)
    
    # Setup heatmap
    # Create a fixed aspect ratio heatmap that aligns with the line plot
    heatmap = ax2.imshow(np.zeros((1, len(x))), 
                        aspect='auto',  # Let the aspect ratio adjust to the figure size
                        extent=[0, L, 0, 1],  # [left, right, bottom, top]
                        cmap='hot', 
                        vmin=ymin, 
                        vmax=ymax,
                        interpolation='nearest')  # Use nearest neighbor interpolation for sharp edges
    
    # Ensure the heatmap's x-axis matches the line plot
    ax2.set_xlim(0, L)
    ax2.set_ylim(0, 1)
    ax2.set_yticks([])  # Hide y-axis ticks for heatmap
    ax2.set_xlabel("Position (x)")
    ax2.set_ylabel("")
    
    # Add colorbar
    cbar = plt.colorbar(heatmap, ax=ax2, orientation='horizontal', pad=0.1)
    cbar.set_label("Temperature")
    
    def init():
        line.set_data([], [])
        time_text.set_text('')
        heatmap.set_data(np.zeros((1, len(x))))
        return line, time_text, heatmap
    ...
    def update(frame):
        ...
        # Update heatmap
        heatmap_data = np.zeros((1, len(x)))
        heatmap_data[0, :] = current_data['Temperature']
...
def main():
    ...
    parser.add_argument('--output', help='Output GIF file name (default: heat_conduction_2d_TIMESTAMP.gif)')
